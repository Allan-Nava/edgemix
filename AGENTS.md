...
[CLAUDE.md](CLAUDE.md) is a copy of this brief for Claude Code — when they
...
- **Releases**: every release is a tagged `vX.Y.Z` with a new `CHANGELOG.md`
  ...
  for fixes. The tag is what runs
  [`release.yml`](.github/workflows/release.yml): six archives with
  `SHA256SUMS`, a sigstore attestation over each, the `ghcr.io` image, release
  notes lifted from the CHANGELOG section for that version, and the Homebrew
  formula generated by `scripts/brew.sh` from the checksums that run produced —
  so the CHANGELOG section has to exist *before* the tag, and a formula is never
  hand-edited. A `workflow_dispatch` run rehearses all of it without
  publishing. **Never `git push`**, tags included — that is the maintainer's
  call, and here it is also the publish button. No `Co-Authored-By` trailers.
