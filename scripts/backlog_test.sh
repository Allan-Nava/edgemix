...
# row in three the first time an item title contained a `|` (EM-63): the
...
- [x] **EM-1 — Shipped and high**: nothing more to do here.
  <!-- em: prio=high size=S labels=parser ver=0.1.0 -->
- [ ] **EM-2 — `--profile apple|dash-if|none`**: a pipe is legal in a title, and
  EM-63 is named exactly like this.
  <!-- em: prio=med size=M labels=cli -->

## M2 — Later things <!-- ms: target=v0.2.0 phase=later -->

- [ ] **EM-3 — An em dash — right here**: it has to survive the round trip.
  <!-- em: prio=low size=L labels=docs,tests -->
- [ ] **EM-4 — Open and high, listed after a low one**: priority decides the
...
checks=$((checks + 1))
if [ "$(grep -c '^| \*\*EM-2\*\*' "$tmp/ROADMAP.md" 2>/dev/null || grep -c 'EM-2' "$tmp/ROADMAP.md")" -ge 1 ]; then
	echo "ok   the row carrying a pipe is still one row"
else
	fail "EM-2 is missing from the roadmap entirely"
fi

# Every row of one markdown table has the same number of cells, and an escaped
# pipe is not one of them. This is the assertion EM-63 would have failed: the
...
checks=$((checks + 1))
if [ "$(grep -n 'EM-4' "$tmp/ROADMAP.md" | head -1 | cut -d: -f1)" \
	...
	fail "EM-4 (high) is listed after EM-3 (low)"
fi
...
assert_contains "next leads with the highest priority open item" "EM-4" "$tmp/next.txt"
assert_absent "next does not offer work that is already done" "EM-1" "$tmp/next.txt"
checks=$((checks + 1))
...
# An id used twice: two commits reference EM-2 and mean different work.
printf '%s\n%s\n' "$head" '- [ ] **EM-1 — One**: a.
  <!-- em: prio=med size=S labels=cli -->
- [ ] **EM-1 — Two**: b.
  <!-- em: prio=med size=S labels=cli -->' >"$tmp/dup.md"
assert_fails "a duplicate id is rejected" "$tmp/dup.md" "EM-1"
...
printf '%s\n%s\n' "$head" '- [ ] **EM-1 — One**: a.
  <!-- em: prio=med size=S labels=cli -->
- [ ] **EM-3 — Three**: the gap where EM-2 was.
  <!-- em: prio=med size=S labels=cli -->' >"$tmp/gap.md"
assert_fails "a gap in the sequence is rejected" "$tmp/gap.md" "EM-2"
...
printf '%s\n%s\n' "$head" '- [ ] **EM-1 — One**: a.
  <!-- em: prio=urgent size=S labels=cli -->' >"$tmp/prio.md"
assert_fails "an unknown priority is rejected" "$tmp/prio.md" "prio"

printf '%s\n%s\n' "$head" '- [ ] **EM-1 — One**: a.
  <!-- em: prio=med size=XXL labels=cli -->' >"$tmp/size.md"
...
printf '%s\n%s\n' "$head" '- [ ] **EM-1 — One**: a.' >"$tmp/nometa.md"
assert_fails "an item with no metadata is rejected" "$tmp/nometa.md" "EM-1"
...
printf '%s\n%s\n' "$head" '- [x] **EM-1 — One**: shipped, apparently.
  <!-- em: prio=med size=S labels=cli -->' >"$tmp/nover.md"
...
" '- [ ] **EM-1 — One**: still open.
  <!-- em: prio=med size=S labels=cli -->' >"$tmp/phase.md"
