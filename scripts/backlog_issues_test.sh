...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/edgemix-backlog-test.XXXXXX")
...
- [x] **EM-1 — Already shipped and already closed**: nothing to do here.
  <!-- em: prio=high size=S labels=parser ver=0.1.0 -->
- [x] **EM-2 — Shipped but its issue is still open**: the sync has to close it.
  <!-- em: prio=med size=M labels=check ver=0.1.0 -->
- [x] **EM-3 — Shipped and never had an issue**: must not be created now.
  <!-- em: prio=low size=S labels=docs ver=0.1.0 -->

## M2 — Planned things <!-- ms: target=v0.2.0 phase=now -->

- [ ] **EM-4 — Open with an issue already open**: leave it alone.
  <!-- em: prio=high size=L labels=check,parser -->
- [ ] **EM-5 — Open with no issue yet**: create it.
  <!-- em: prio=med size=M labels=output -->
- [ ] **EM-6 — Open but its issue was closed**: reopen it.
  <!-- em: prio=low size=S labels=tests -->
- [ ] **EM-7 — `--profile apple|dash-if|none`**: a pipe is legal in a title, and
  the real EM-63 is named exactly like this.
  <!-- em: prio=med size=S labels=cli -->
EOF
...
# which opened 44 issues titled "EM-n — " cannot come back unnoticed.
cat >"$tmp/snapshot.tsv" <<'EOF'
EM-1	101	closed	EM-1 — Already shipped and already closed
EM-2	102	open	EM-2 — Shipped but its issue is still open
EM-4	104	open	EM-4 — Open with an issue already open
EM-6	106	closed	EM-6 — Open but its issue was closed
EOF
...
# ---------------------------------------------------------------------------
assert_absent "a shipped item with no issue is not created" "CREATE	EM-3" "$tmp/actions.txt"
...
EM-1	101	closed	EM-1 — Already shipped and already closed
EM-2	102	closed	EM-2 — Shipped but its issue is still open
EM-3	103	closed	EM-3 — Shipped and never had an issue
EM-4	104	open	EM-4 — Open with an issue already open
EM-5	105	open	EM-5 — Open with no issue yet
EM-6	106	open	EM-6 — Open but its issue was closed
EM-7	107	open	EM-7 — `--profile apple|dash-if|none`
EOF
...
# public issues named "EM-n — " with nothing after the dash. `ver` is empty for
# every open item, so the mistake was invisible to a plan-level or body-level test.
# ---------------------------------------------------------------------------
sh "$script" issues --title EM-5 >"$tmp/title.txt"
assert_contains "the title carries the item's name" "EM-5 — Open with no issue yet" "$tmp/title.txt"
...
# A pipe in the title must survive intact: it was the field separator until EM-63
# proved a title can contain one, and the issue was published truncated.
sh "$script" issues --title EM-7 >"$tmp/pipe.txt"
assert_contains "a pipe in the title survives" 'EM-7 — `--profile apple|dash-if|none`' "$tmp/pipe.txt"
...
EM-4	104	open	EM-4 — Some older name
EM-5	105	open	EM-5 — Open with no issue yet
EM-6	106	open	EM-6 — Open but its issue was closed
EOF
...
assert_contains "a drifted title is corrected" "RETITLE	EM-4	104" "$tmp/actions5.txt"
assert_absent "correcting a title does not close the issue" "CLOSE	EM-4" "$tmp/actions5.txt"
assert_contains "a matching title is left alone" "OK	EM-5	105" "$tmp/actions5.txt"
...
assert_contains "the filter keeps its milestone" "CREATE	EM-5" "$tmp/actions3.txt"
assert_absent "the filter excludes other milestones" "EM-2" "$tmp/actions3.txt"

# ---------------------------------------------------------------------------
# The created issue carries what a reader needs: the backlog prose, the id, the
# milestone and its target.
# ---------------------------------------------------------------------------
sh "$script" issues --body EM-5 >"$tmp/body.txt"
assert_contains "the body carries the item's prose" "create it." "$tmp/body.txt"
assert_contains "the body names the id" "EM-5" "$tmp/body.txt"
...
- [ ] **EM-1 — Missing its metadata comment**: no trailing sc comment at all.
EOF
checks=$((checks + 1))
