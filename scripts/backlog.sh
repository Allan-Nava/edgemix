...
# stable EM-n id and a trailing `<!-- em: ... -->` metadata comment. ROADMAP.md is
...
#     --apply | --milestones M11,M12 | --body EM-n | --title EM-n
#
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/edgemix-backlog.XXXXXX")
...
lint() {
	...
	END {
		for (i = 1; i <= max; i++)
			if (!("EM-" i in seen)) err("EM-" i " is missing — ids run 1..N with no gaps; retire an item with status done, never by deleting it")
...
issue_body() {
	awk -v want="$1" -F'\t' \
		...
		-v repo_blob="https://github.com/Allan-Nava/edgemix/blob/main" '
	...
	$1 == "I" && $3 == want {
		...
		print ""
		print "`BACKLOG.md` is the single source of truth: it carries the stable `EM-n` id that commits and the CHANGELOG reference, and [ROADMAP.md](" repo_blob "/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by `scripts/backlog.sh issues --apply`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit."
	}
...
# EM-63 is literally named `--profile apple|dash-if|none`. Splitting on it truncated
...
# titled "EM-n — " with nothing after the dash: `ver` is empty for an open item, so
...
ensure_labels() {
	...
	for spec in \
		...
		"integration|bfd4f2|Using edgemix from other systems" \
...
issues_cmd() {
	...
	while [ $# -gt 0 ]; do
		...
		--title)
			...
			title_of="${1:-}"
			[ -n "$title_of" ] || {
				echo "backlog.sh: --title needs an EM-n id" >&2
				exit 2
			}
			;;
		--body)
			...
			body_of="${1:-}"
			[ -n "$body_of" ] || {
				echo "backlog.sh: --body needs an EM-n id" >&2
				exit 2
			}
			;;
		*)
...
*)
	...
	echo "       scripts/backlog.sh issues [--apply] [--milestones M11,M12] [--body EM-n]" >&2
	exit 2
	;;
esac
